...
export function DukaProvider({ children }: { children: ReactNode }) {
  ...
  useEffect(() => {
    ...
    async function init() {
      ...
      const { data:row } = await supabase.from("merchants").select("*").eq("user_id",session.user.id).single();
      if (row) {
  ...
  const login = useCallback(async (phone:string, role:SessionRole="owner") => {
    ...
    let { data:row } = await supabase.from("merchants").select("*").eq("user_id",user.id).single();
    if(!row){
