      ...
      <div style={{ padding: 16 }}>
        {products.length === 0 ? (
          <div style={{ padding: "60px 16px", textAlign: "center" }}>
            ...
            <button className="dy-btn dy-btn-primary" style={{ marginTop: 18, width: "auto", padding: "12px 22px" }} onClick={startAdd}>{t("＋ Ongeza Bidhaa ya Kwanza", "＋ Add First Product")}</button>
          </div>
        ) : (
            ...
            {products.map(p => (
              <div key={p.id} className="dy-card" style={{ padding: 10, display: "flex", flexDirection: "column", gap: 8 }}>
                ...
                <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr 1fr", gap: 4 }}>
                  <IconBtn title={t("Hariri", "Edit")} onClick={() => startEdit(p)}>✏️</IconBtn>
                  <IconBtn title={t("Geuza", "Toggle")} onClick={() => { toggleProduct(p.id); toast(p.isAvailable ? t("Imefichwa", "Hidden") : t("Inapatikana", "Available")); }}>🔀</IconBtn>
                  <IconBtn title={t("Futa", "Delete")} color="#E74C3C" onClick={() => { if (confirm(t(`Futa "${p.name}"?`, `Delete "${p.name}"?`))) { deleteProduct(p.id); toast(t("Imefutwa", "Deleted")); } }}>🗑</IconBtn>
                </div>
              </div>
            ))}
          </div>
  ...
  return (
    <Modal open={open} onClose={onClose} title={editing ? t("Hariri Bidhaa", "Edit Product") : t("Ongeza Bidhaa Mpya", "Add New Product")}>
      <div style={{ display: "grid", gap: 12 }}>
        ...
        }}>{editing ? t("💾 Hifadhi Mabadiliko", "💾 Save Changes") : t("✅ Ongeza Bidhaa", "✅ Add Product")}</button>
      </div>
    </Modal>
  );
}
